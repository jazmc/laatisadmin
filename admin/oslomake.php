...
// jos seuraavan tilaisuuden ilmoittautumisaika ei ole vielä alkanut 
    ...
    <p>Ilmoittautuminen seuraavaan tilaisuuteen aukeaa <?php echo strtolower($kuukausi[date('m', strtotime($seuraavatil['IlmoAlku']))]) . "n " . date('j', strtotime($seuraavatil['IlmoAlku'])); ?>. päivänä.</p>
...
<?php
    ...
    <h2>Ilmoittautuminen, <?php if (count($avoimet) == 1) {
                                echo (!empty($seuraavatil['Otsikko']) ? $seuraavatil['Otsikko'] : strtolower($kuukausi[date('m', strtotime($seuraavatil['Pvm']))]) . "n tilaisuus");
                                echo "&ndash; VIP " . (date('Y-m-d') == $seuraavatil['IlmoLoppu']) ? "tänään!" : date('d.m.', strtotime($seuraavatil['IlmoLoppu']));
                            } else if (count($avoimet) > 1) {
                                echo "useita tilaisuuksia avoinna";
                            }
                            ?></h2>
    <p>Ilmoittautuminen on sulkeutunut. Ilmoittautuminen seuraavaan tilaisuuteen aukeaa <?php echo strtolower($kuukausi[date('m', strtotime($sitaseuraava['IlmoAlku']))]) . "n " . date('j', strtotime($sitaseuraava['IlmoAlku'])); ?>. päivänä.</p>
    ...
    <form action="admin/lisaaosallistuja.php" method="POST" class="mt-4">
        <div class="row">
            ...
            <div class="col input-group mb-3">
                ...
                <input type="text" id="VRL" name="VRL" class="form-control" placeholder="00000" aria-label="00000" aria-describedby="basic-addon1">
                <div class="input-group-append">
                    <button class="btn btn-success" id="aloita" type="button" onclick="tarkistaTuomarius()" disabled>Aloita ilmoittautuminen</button>
                </div>
            </div>
        </div>
